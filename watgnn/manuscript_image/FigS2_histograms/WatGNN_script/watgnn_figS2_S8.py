...
from watgnn_visualization_fig2 import pdb_dict_as_pdb, pdb_dict_as_pdb_new
from watgnn_input_preprocess import transform, merge_pdb_dicts, get_probe
from watgnn_input_suppl import read_paths,read_dataset, read_dataset_simple, read_pdbbind_simple
from watgnn_models import Model
from watgnn_evaluation import eval_dataset, analysis_water, analysis_water_S2, analysis_water_S8
...
def runner(mode='train',dataset_path=None): #pred_per_atom -> n_grid
    ...
    for line in dataset_lines:
        if line.startswith('#'):
            continue
        lsp_comma = line.strip().split(',')
        if len(lsp_comma) == 2:
            chainsel = lsp_comma[1].strip()
        else:
            chainsel = None
        
        lsp = lsp_comma[0].split()
        if len(lsp) == 2:
            dataset.append([lsp[0],chainsel])
            dataset_lig.append(None)
            dataset_pred.append(lsp[1])
        elif len(lsp) == 3:
            dataset.append([lsp[0],chainsel])
            dataset_lig.append(lsp[1])
            dataset_pred.append(lsp[2])
    ...
    elif mode == 'analysis_S2':
        if len(dataset) > 0:           
            #analysis_water_S2(model, dataset, dataset_lig, dataset_pred, config, log_dir = 'gnn_S2_training', log_paths = ['gnn_S2_training.txt','gnn_S2_n_atm_training.txt'] , result_pdb_dir = 'gnn_S2_training',label='None' )
            
            #analysis_water_S2(model, dataset, dataset_lig, dataset_pred, config, log_dir = 'gnn_S2_validation', log_paths = ['gnn_S2_validation.txt','gnn_S2_n_atm_validation.txt'] , result_pdb_dir = 'gnn_S2_validation',label='None' )
            
            analysis_water_S2(model, dataset, dataset_lig, dataset_pred, config, log_dir = 'gnn_S2_test', log_paths = ['gnn_S2_test.txt','gnn_S2_n_atm_test.txt'] , result_pdb_dir = 'gnn_S2_test',label='None' )
            
            
    elif mode == 'analysis_S8':
        if len(dataset) > 0:
            # water score at pdb = 100 * actual score (to match with b-factor format)            
            #analysis_water_S8(model, dataset, dataset_lig, dataset_pred, config, log_dir = 'gnn_S8_training', log_paths = ['gnn_S8_training.txt','gnn_S8_n_atm_training.txt'] , result_pdb_dir = 'gnn_S8_training',label='None',water_scorecut=65.0 )
            
            #analysis_water_S8(model, dataset, dataset_lig, dataset_pred, config, log_dir = 'gnn_S8_validation', log_paths = ['gnn_S8_validation.txt','gnn_S8_n_atm_validation.txt'] , result_pdb_dir = 'gnn_S8_validation',label='None',water_scorecut=65.0 )
            
            analysis_water_S8(model, dataset, dataset_lig, dataset_pred, config, log_dir = 'gnn_S8_test', log_paths = ['gnn_S8_test.txt','gnn_S8_n_atm_test.txt'] , result_pdb_dir = 'gnn_S8_test',label='None',water_scorecut=65.0 )


def main():
    if len(sys.argv) <= 1:
        print("usage: watgnn.py [dataset file path]")
        print("dataset file structure: [pdb file path (relative path)] [mol2 file path (relative path, optional)] [water prediction pdb file path (relative path)] ")
    else:
        dataset_path = sys.argv[1] 
        #runner(mode = 'eval', dataset_path=dataset_path)
        runner(mode = 'analysis_S2', dataset_path=dataset_path)
        runner(mode = 'analysis_S8', dataset_path=dataset_path)
