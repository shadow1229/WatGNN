...
def pdb_dict_as_pdb_new (pdb_dict, dgl_graph=None, outpath='x.pdb',bildpath=None):
    ...
    resno_write_list = [165] #ducttape for 5IAI!
    resno_write_protein_list = [164,165] #ducttape for 5IAI!
    if bildpath == None:
    ...
    probe_mask_list = pdb_dict['polar_mask_list']
    
    p_base_list = pdb_dict['p_base_list']
    center_pos_list = pdb_dict['center_pos_list']
    v1_list = pdb_dict['v1_list']
    ...
    for i in range(n_atm):
        ...
        if resno_write_list != None:
            if resno not in resno_write_protein_list: #to write C atom in previous residue
                continue 
    ...
    for i in range(n_atm):
        ...
        atmname = atmname_list[i]
        if resno_write_list != None:
            if resno not in resno_write_list: 
                continue 
        if polar_mask_list[i] == 1:
            pbs_pos = p_base_list[i]
            pbs_txt = 'HETATM%5d %4s %3s B%4d    %8.3f%8.3f%8.3f\n'%(10000+atmno," H  ", "PBS", 1000+resno,*pbs_pos)
            pdb_f.write(pbs_txt)         
    ...
    for i in range(n_atm):
        ...
        if resno_write_list != None:
            if resno not in resno_write_list: 
                continue 
        if polar_mask_list[i] == 1:
            cnt_pos = center_pos_list[i]
            cnt_txt = 'HETATM%5d %4s %3s C%4d    %8.3f%8.3f%8.3f\n'%(20000+atmno," H  ", "CNT", 2000+resno,*cnt_pos)   
            pdb_f.write(cnt_txt)                
    ...
    for ax in range(3):
        ...
        bild_axis_f.write('.transparency 0.0\n') 
        for i in range(n_atm):
            resno   = resno_list[i] 
            if resno_write_list != None:
                if resno not in resno_write_list: 
            ...
            if polar_mask_list[i] == 1:
                ...
                vec_end   = (2.25*axis_list[i][ax] + pos_list_torch[i]).tolist()
                bild_axis_f.write('.arrow %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f 0.05 0.2\n'%(*vec_start, *vec_end))


    # save polar_vec
    """
    bild_axis_f.write('.color 0.5 0.5 0.5\n') #gray color
    bild_axis_f.write('.transparency 0.7\n') 
    for i in range(n_atm):
        resno   = resno_list[i] #ducttape for 5IAI!
        if resno_write_list != None:
            if resno not in resno_write_list: 
                continue 
        if polar_mask_list[i] == 1:
            vec_start = p_base_list[i]
            vec_end   = (2.25*axis_list[i][0] + pos_list_torch[i]).tolist() 
            bild_axis_f.write('.arrow %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f 0.05 0.2\n'%(*vec_start, *vec_end))
    """        
            
            
    # save v1
    bild_axis_f.write('.color 0.5 0.5 0.0\n') #yellow color
    bild_axis_f.write('.transparency 0.0\n') 
    for i in range(n_atm):
        resno   = resno_list[i] #ducttape for 5IAI!
        if resno_write_list != None:
            if resno not in resno_write_list: 
                continue 
        if polar_mask_list[i] == 1:
            vec_start = center_pos_list[i]
            vec_end   = pos_list_torch[i].tolist()
            bild_axis_f.write('.arrow %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f 0.05 0.2\n'%(*vec_start, *vec_end))
    ...
    for i in range(n_atm):
        ...
        for j in range(grid_diff.shape[1]):
            ...
            txt = 'HETATM%5d  H   PRB A%4d    %8.3f%8.3f%8.3f\n'%(prb_idx%100000,resno,*vec)
            prb_f.write(txt)
