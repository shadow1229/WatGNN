...
def read_pdb_pred_water(fpath_chain,water_scorecut=0.1):
    #fpath_chain : fpath_chain in read_pdb
    #for fpath_chain in paths:
    ...
    for line in lines:
        if line.startswith('ATOM') or line.startswith('HETATM'):
            ...
            alt   = line[16]
            resname = line[17:20]
            score = float(line[60:66]) #bfac part is used as score
            if alt not in [' ','A']:
                continue
            if resname in res_dict.keys():
                rid = res_dict[resname]
            ...
            if (not ispro) and (not iswat):
                ...
                if score >= water_scorecut: # #bfac part is used as score. watch out >= was used instead of <, sinc it's a score but not B-factor.
                    if chain_sel == None or chain == chain_sel: #contain other chain water??? TODO
                        water['water_pos_tmp'].append(vec)
                    else:
                        water['water_pos_other'].append(vec)
...
def read_paths_suppl(paths,predpath,water_cutoff = 4.0,grid_start=-4.5, interval=3.0, n_grid=3, bcut=40, water_scorecut=0.1 ,is_eval=False):
    #water: bcut: 40 
    ...
    pdb_container_pred_wat = read_pdb_pred_water([predpath,None],water_scorecut=water_scorecut)
    water_pos_tmp   = pdb_container_pred_wat['water']['water_pos_tmp']
    water_pos_other = pdb_container_pred_wat['water']['water_pos_other']
    ...
    # for fig2 purpose
    p_base_list = np.zeros( (len(pos_list),3) )  #[vector for atom in atm_list]
    center_pos_list = np.zeros( (len(pos_list),3) )  #[vector for atom in atm_list]
    v1_list =np.zeros( (len(pos_list),3) )  #[vector for atom in atm_list]
    ...
    for ent_idx in range(len(entity_firstres_list)):
        ...
        for res_idx in range(res_first, res_last+1):
            ...
            for atm_idx in range(atm_first,atm_last+1):
                ...
                p_base_pos = np.zeros(3,) #DOES NOT APPEAR IN ACTUAL CODE
                center_pos = np.zeros(3,) #DOES NOT APPEAR IN ACTUAL CODE
                ...
                for j in range(n_atm_prev_to_next):
                    ...
                    if atm_dist[j] < 0.01: #remove self #DOES NOT APPEAR IN ACTUAL CODE - minor bug, does not affect performance
                        continue #DOES NOT APPEAR IN ACTUAL CODE - minor bug, does not affect performance
                    if atm_dist[j] < break_cutoff:
                        pos_neighs.append(pos_list[atm_jdx])

                for pos_neigh in pos_neighs:
                    polar_vec += (pos_i - pos_neigh)
                    p_base_pos += pos_neigh / float(len(pos_neighs)) #DOES NOT APPEAR IN ACTUAL CODE
                ...
                for j in range(atm_first,atm_last+1):
                    ...
                    center_pos += (pos_list[j]) / float(len(range(atm_first,atm_last+1))) #DOES NOT APPEAR IN ACTUAL CODE
                ...
                if polar_mask_list[atm_idx] == 1:
                    ...
                    p_base_list[atm_idx] = p_base_pos #DOES NOT APPEAR IN ACTUAL CODE
                    center_pos_list[atm_idx] = center_pos #DOES NOT APPEAR IN ACTUAL CODE
                    v1_list[atm_idx] = v1 #DOES NOT APPEAR IN ACTUAL CODE
    ...
    result =  { 'entities':entities, #new
                ...
                'p_base_list':p_base_list, #position for building polar vector DOES NOT APPEAR IN ACTUAL CODE
                'center_pos_list':center_pos_list, #position for building aux vector DOES NOT APPEAR IN ACTUAL CODE
                'v1_list':v1_list, #vector for building e1 DOES NOT APPEAR IN ACTUAL CODE
                }
    return result
